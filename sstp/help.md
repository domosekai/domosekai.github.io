...
These items are needed for a profile:
  ...
  - Virtual Hub name (SoftEther VPN)
...
- **Server** 
  ...
  *Required*
  ...
  However, if your DNS provider is unreliable, or you want to enforce connecting via IPv4 or IPv6 (the server is dual-stack), you can use an IP address instead. 
...
- **Hostname**
  ...
  If you entered an IP address in the server field, please put the hostname in this field that matches the server certificate, otherwise TLS validation will fail.
...
- **TLS Server Trust Evaluation**
  ...
  If the server uses non-public certificates, additional steps may be required to pass TLS validation. See below for more information.
...
- **Virtual Hub**
  ...
  *SoftEther Only*
  ...
  The most common names are `default` and `vpn`. If you are using VPN Gate service, the hub name is `vpngate`.
...
- **UDP Acceleration**
  ...
  *SoftEther Only*
  ...
  Whether to enable the UDP acceleration feature. Available in iOS 13+ and SoftEther Server 4.30+.

- **Authentication Method**

  Choose either password or certificate. Authentication by certificate is supported in iOS 12 and above.
...
- **Username** 
  ...
  *Required*
  ...
  For SoftEther users connecting with SSTP protocol, the Virtual Hub name should be appended to the account in the form `user@VPNHUB`, unless the default Virtual Hub is being used.
...
- **Enable PAP / CHAP**
  ...
  *SSTP Only*
  ...
  Enable if the server only supports these obsolete authentication methods. Only use them when necessary.
  
- **Tunnel IP Version**

  Choose the IP version inside the VPN tunnel. For IPv6 tunnels, only stateless configuration is currently supported (no DHCPv6 support).

- **Static IP / DNS**

  Fill in manual IP and DNS settings as instructed by your administrator.
...
- **Add Default Route**
  ...
  In iOS system VPN the option is called *Send All Traffic*.
...
- **Switch to Wi-Fi When Available**
  ...
  When an existing VPN session is on mobile network and the device connects to Wi-Fi, by default the session will stay on mobile data. Enable this option to allow upgrading to Wi-Fi.
  
  Note that the upgrade is controlled by iOS and might not always work. Especially if the current connection is made over IPv6 but the Wi-Fi is IPv4 only, there is chance that the upgrade will not occur. 
...
- **Stay Connected During Sleep**
  ...
  Especially we have noticed that when both mobile data and WiFi is on, connections over WiFi cannot stay on during sleep. Switching off mobile data may help.
...
- Signing algorithm should be in the SHA-2 family (such as SHA256)
- DNS name should be presented in Subject Alternative Name (CommonName is no longer trusted)
...
For security reasons, certificates signed by public CAs should be used. However if that’s not possible, you can ask your administrator for the root CA certificate and both install and trust it in your device. 
...
- iTunes File Sharing on a computer
...
When the certificate is on your device, you can import it into the app by tapping *Import* and navigate to the folder. You need to enter the password and give the certificate a distinct description. 

The certificate (and its private key) will be saved in the keychain. For security reasons, please delete the file after import finished or save in a secure place.
...
According to iOS policy, certificates with the same serial number from the same issuer are considered the same. Please ask the administrator to generate the certificate in new serial number.
...
**Note:** The order is important. If you place Rule 2 before Rule 1, Rule 1 will never run because Rule 2 matches any condition.
You can change the order by tapping *Edit* and drag into your desired order.
...
For example, you can setup the following rule that if some internal website is being requested (not resolvable on the current network), corporate VPN is started.
