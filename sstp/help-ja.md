...
現在のSoftEtherプロトコルによれば、既存のセッションの新しいエンドポイントを再ネゴシエートする方法がないため、UDP高速化がNATトラバーサルモードで動作する場合、これは仕方ないです。
...
SoftEther VPN Serverによって自動的に作成された証明書は、iOS 13の要件を満たしていないため、この[ガイド](sevpn-cert-ja.html)に従って再作成する必要があります。
