...
- **ユーザ名**
  ...
  *必須*
  ...
  証明書認証を使用するSSTPユーザの場合、証明書のCommon Name記載と一致する限り、ユーザ名を省略できます。
...
- **証明書**
  ...
  *証明書認証に必要*
  ...
  証明書をインポートする方法については、以下を参照してください。現在、RSA証明書のみがサポートされています。
...
証明書認証を使用するには、まず証明書をインポートすることです。現在、アプリはPKCS #12形式（.p12または.pfxファイル）に格納されたRSA証明書のみをサポートしています。
