...
- **コネクション数**
  ...
  *SoftEtherのみ*
  ...
  最大32の接続をサポートしていますが、2本から試してみることをお勧めします。接続が増えると、より多くのメモリが消費されます。重い負荷では使用しないでください。
...
- **静的IP/DNS設定**
  ...
  管理者からの指示がある場合は、手動でIPとDNS設定を設定できます。
...
まずは一連の規則を作成する必要があります。iOSが順番に評価を行う、最初に該当したルールのみが適用されます。
...
コネクション評価は、接続ごとで宛先を評価してアクションを実行する特別なアクションです。
